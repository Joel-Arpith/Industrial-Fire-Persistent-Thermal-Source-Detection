...
| script | what it does |
|---|---|
| `build_corpus.py` | Pulls a multi-region, multi-season FIRMS corpus. Recent data alone yields 5 of the 7 label classes: `agricultural_burn` needs the Oct–Nov stubble window and `known_false_positive` needs a solar park in bbox. |
...
```bash
python tools/build_corpus.py          # FIRMS -> data/firms_corpus.csv
